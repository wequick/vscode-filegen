...
async function handleCommand(type: number, args: any) {
	...
	var header_preset       = vscode.workspace.getConfiguration().get("alcpp.creator.headerFileContentPreset")    as string;
	var source_file_preset  = vscode.workspace.getConfiguration().get("alcpp.creator.sourceFileContentPreset")    as string;
	var source_file_name    = vscode.workspace.getConfiguration().get("alcpp.creator.sourceFileNamePreset")       as string;
	var header_file_name    = vscode.workspace.getConfiguration().get("alcpp.creator.headerFileNamePreset")       as string;
	var unittest_file_name    = vscode.workspace.getConfiguration().get("alcpp.creator.unittestFileNamePreset")   as string;
	const root = vscode_helper.workspace() || "";
	var creator = new class_creator(type, root, res as string, header_preset, source_file_preset, dir_h.dir(), source_file_name, header_file_name, unittest_file_name)
	await creator.parse()
